object KDTreeTest extends App {
  def test[T](haystack: Seq[Seq[T]], needles: Seq[T]*)(implicit num: Numeric[T]): Unit = {
    println
    val tree: Option[KDTree.KDNode[T]] = KDTree(haystack)
    if (haystack.size < 20) tree.foreach((x: KDTree.KDNode[T]) => println(x))
